...
export function CmdKDialog() {
	...
	const searchResultItems = useMemo(
		() =>
			searchResults?.map((item, idx) => (
				<Command.Item
					className="dark:border-dark-100 [&[aria-selected]]:ring-width-2 [&[aria-selected]]:ring-blurple dark:active:bg-dark-200 dark:hover:bg-dark-300 my-1 flex transform-gpu cursor-pointer select-none appearance-none flex-row place-content-center rounded bg-transparent px-4 py-2 text-base font-semibold leading-none text-black outline-none hover:bg-neutral-100 active:translate-y-px active:bg-neutral-200 dark:text-white [&[aria-selected]]:ring"
					key={`${item.id}-${idx}`}
					onSelect={() => {
	...
	return (
		<Dialog className="fixed left-1/2 top-1/4 z-50 -translate-x-1/2" state={dialog!}>
			<Command
				className="min-w-xs border-light-900 sm:min-w-lg dark:border-dark-100 dark:bg-dark/50 max-w-xs rounded border bg-white/50 shadow backdrop-blur-md sm:max-w-lg"
				label="Command Menu"
				shouldFilter={false}
			>
				<Command.Input
					className="border-light-900 rounded-b-0 caret-blurple dark:border-dark-100 dark:bg-dark/50 placeholder:text-dark-300/75 w-full rounded border-0 border-b bg-white/50 p-4 text-lg outline-none dark:placeholder:text-white/75"
