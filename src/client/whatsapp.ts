...
import { type PhoneNumber } from './doc/IPhoneNumber'
import { ClientStatusEnum } from './doc/IClientStatus'
...
export class WhatsappClient extends EventEmitter {
	...
	constructor(options: ClientOptions) {
		...
		if (options.phone_number || options.phone_number_id)
			this.setPhoneNumberInUse(options.phone_number || options.phone_number_id)
		this.version = options.version
		this.axiosClient = createAxiosInstance(
			`${WhatsappClient.baseUrl}/${options.version}`,
			this.token
		)
	 ...
	 */
	isReady(): boolean {
		if (this.status === ClientStatusEnum.Ready) return true
