...
import { type VerificationTypeEnum } from './doc/IVerificationType'
...
export class PhoneNumberManager extends BaseManager {
	 ...
	 * NOTE: You are required to verify your phone number before registering with the api
	 * @param {string} phoneNumberId
	 * @param {VerificationTypeEnum} type\
	 * @param {string} language
	 * @returns
	 */
	async requestVerification(phoneNumberId: string, type: VerificationTypeEnum, language: string) {
		const data = {
			coe_method: type,
			language
		}
	...
	async getPhoneNumbers() {
		const response = await this.client.getRequestClient.get(
			`/${this.client.getBusinessAccountId}`
		)
