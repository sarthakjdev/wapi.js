...
export async function createWhatsappBot(options: {
    ...
    const { directory, doInstallPackage, isTypescriptEnabled, packageManagerInUse } = options
    ...
    if (bun) {
        await cp(
            new URL(
                `../template/Bun/${isTypescriptEnabled ? 'typeScript' : 'javaScript'}/package.json`,
                import.meta.url
            ),
            `${root}/package.json`
        )
    ...
    if (!deno) {
        const newPackageJSON = await readFile('./package.json', {
            ...
            newStr = newStr.replaceAll('[REPLACE_IMPORT_EXT]', isTypescriptEnabled ? 'ts' : 'js')
            return newStr
        })
